...
bool8 ScrCmd_fwdweekday(struct ScriptContext *ctx)
{
    ...
    if (gSaveBlock1Ptr->tx_Features_RTCType == 1) // real rtc
        return FALSE;
...
bool8 ScrCmd_removenamedmon(struct ScriptContext *ctx)
{
    ...
    for (u8 i = 0; i < PARTY_SIZE; i++)
    {
        if (GetMonData(&gParties[B_TRAINER_PLAYER][0], MON_DATA_SPECIES) != SPECIES_NONE)
        {
            ...
            {
                u16 heldItem = GetMonData(&gParties[B_TRAINER_PLAYER][0], MON_DATA_HELD_ITEM);
                ...
                if (giftId == 2)
                {
                    u8 friendship = GetMonData(&gParties[B_TRAINER_PLAYER][0], MON_DATA_FRIENDSHIP);
                    if (friendship > 200)
                    {
...
bool8 ScrCmd_baobacheckmon(struct ScriptContext *ctx)
{
    ...
    if (GetMonData(&gParties[B_TRAINER_PLAYER][0], MON_DATA_SPECIES, NULL) == SPECIES_NONE)
        return FALSE;
    if (GetMonData(&gParties[B_TRAINER_PLAYER][0], MON_DATA_IS_EGG, NULL))
        return FALSE;

    species = GetMonData(&gParties[B_TRAINER_PLAYER][0], MON_DATA_SPECIES, NULL);
...
static u8 GiveOddEgg_Internal(u16 species, bool8 forceShiny, bool8 allow14PercentShiny)
{
    ...
    CreateMonLegacy(&mon, species, P_EGG_HATCH_LEVEL, USE_RANDOM_IVS, TRUE, pid, OT_ID_PLAYER_ID, 0);
    ball = BALL_POKE;
...
bool8 ScrCmd_checkpartymonlevel(struct ScriptContext *ctx)
{
    ...
    gSpecialVar_Result = PARTY_SIZE;
    struct Pokemon *pokemon = &gParties[B_TRAINER_PLAYER][0];
