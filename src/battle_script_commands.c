...
static void Cmd_tryfaintmon(void)
{
    ...
    else
    {
         ...
         && !(gBattleStruct->battlerState[battler].notOnField))
        {
            ...
            {
                gBattlescriptCurrInstr = BattleScript_FaintBattlerNuzlocke;
            }
            else
            {
...
static void Cmd_getexp(void)
{
        ...
        if (gBattleControllerExecFlags == 0)
        {
            ...
            else
            {
                ...
                if (IsValidForBattle(&gParties[B_TRAINER_PLAYER][*expMonId]))
                {
                    ...
                    u32 species, level; 
                    species = GetMonData(&gParties[B_TRAINER_PLAYER][*expMonId], MON_DATA_SPECIES_OR_EGG);
                    //DebugPrintf("Species: %d", species);
                    level =  GetMonData(&gParties[B_TRAINER_PLAYER][*expMonId], MON_DATA_LEVEL);
    ...
    case 6: // check if wild Pokémon has a hold item after fainting
         if (gBattleStruct->wildVictorySong && gBattleMons[gBattlerFainted].item != ITEM_NONE && (gSaveBlock1Ptr->tx_Features_WildMonDropItems == 1))
         {
    ...
    if ((IsNuzlockeActive()) && !(gBattleTypeFlags & BATTLE_TYPE_TRAINER))
        {
            ...
            if ((!isShiny) || gSaveBlock1Ptr->tx_Nuzlocke_ShinyClause == 1) //if the player decides to disable shiny clause
            {
