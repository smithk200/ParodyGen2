...
s32 GetAdjustedDamage(struct DamageContext *ctx, s32 damage)
{
    ...
    else if ((gSaveBlock1Ptr->tx_Mode_Sturdy == 1) && ctx->abilities[ctx->battlerDef] == ABILITY_STURDY && IsBattlerAtMaxHp(ctx->battlerDef))
    {
        enduredHit = TRUE;
...
bool32 IsGen6ExpShareEnabled(void)
{
    if (I_EXP_SHARE_FLAG <= TEMP_FLAGS_END)
        return FALSE;
    if (gSaveBlock2Ptr->optionsEXPShare == 0)
        return TRUE;
