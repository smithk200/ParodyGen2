...
u8 CreateNPCTrainerPartyFromTrainer(struct Pokemon *party, const struct Trainer *trainer, bool32 halfTeam, u32 battleTypeFlags)
{
                                                                        ...
                                                                        | BATTLE_TYPE_TRAINER_HILL)))
    {
        ...
        for (s32 i = 0; i < monsCount; i++)
        {
            ...
            if (gSaveBlock1Ptr->tx_Random_Chaos) //tx_randomizer_and_challenges
                {
                    species = GetSpeciesRandomSeeded(partyData[i].species, TX_RANDOM_T_TRAINER, 0);
                    CreateMon(&party[i], species, GetScaledLevel(partyData[i].lvl), personalityValue, otId);
                    //DebugPrintf("Random, chaotic trainers");
                }
            else if (gSaveBlock1Ptr->tx_Random_Trainer) //tx_randomizer_and_challenges
                {
                    species = GetSpeciesRandomSeeded(partyData[i].species, TX_RANDOM_T_TRAINER, 0);
                    CreateMon(&party[i], species, GetScaledLevel(partyData[i].lvl), personalityValue, otId);
                    //DebugPrintf("Random trainers");
                }
                ...
                || (trainerClass == TRAINER_CLASS_PETER)
                )
                    {
                        ...
                        CreateMon(&party[i], partyData[monIndex].species, trainerLevel, personalityValue, otId);
                        //trainer classes that are not dynamic leveled
                    }
            else
                {
                    ...
                    if (reallevel < 1)
                        reallevel = 1; //making sure the level doesn't go below 1 
                    CreateMon(&party[i], partyData[monIndex].species, reallevel, personalityValue, otId);
                    //every other trainer is dynamic leveled
                }
            ...
            if (gSaveBlock1Ptr->tx_Challenges_TrainerScalingIVs == 0)
               SetMonData(&party[i], MON_DATA_IVS, &(partyData[monIndex].iv));
            if ((gSaveBlock1Ptr->tx_Challenges_TrainerScalingIVs != 0 )&& !FlagGet(FLAG_IS_CHAMPION)) 
            {
            ...
            if (gSaveBlock1Ptr->tx_Challenges_TrainerScalingEVs == 0)
            {
...
static void SpriteCB_MoveWildMonToRight(struct Sprite *sprite)
{
    ...
    {
        if ((B_FAST_INTRO_NO_SLIDE == FALSE || (gSaveBlock2Ptr->optionsFastIntro == 1)) == FALSE && !gTestRunnerHeadless)
            sprite->x2 += 2;
        else
            sprite->x2 = 0;
            ...
            if (gSaveBlock2Ptr->optionsRunType == 1)
                {
                    ...
                    {
                        if (!IsRunningFromBattleImpossible(gBattlerAttacker) && TryRunFromBattle(gBattlerAttacker)){
    ...
    if (gCurrentActionFuncId == B_ACTION_TRY_FINISH || gCurrentActionFuncId == B_ACTION_FINISHED)
    {
        ...
        for (u32 i = 0; i < PARTY_SIZE; i++)
        {
            ...
            if (IsMonDead(&gParties[B_TRAINER_PLAYER][i]) && (gSaveBlock1Ptr->tx_Nuzlocke_Deletion == 1) && !GetMonData(&gParties[B_TRAINER_PLAYER][i], MON_DATA_IS_EGG))
            {
                ...
                for (u32 j = i; j < PARTY_SIZE - 1; j++)
                    gParties[B_TRAINER_PLAYER][j] = gParties[B_TRAINER_PLAYER][j + 1];

                // Clear the last slot (it's now duplicated)
                ZeroMonData(&gParties[B_TRAINER_PLAYER][PARTY_SIZE - 1]);

                // Since we've shifted the party, check this index again
                i--;
            }
