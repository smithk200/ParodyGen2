...
static void Task_MainMenuCheckBattery(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        ...
        else
        {
            ...
            gTasks[taskId].func = Task_DisplayMainMenu; //the rtc thing doesn't seem to work..? Still says clock-based events will no longer occur.
            //Just commented the actual line and replaced it with the gTasks[taskId].func = Task_DisplayMainMenu line to save
            // time when debugging.
        }
    }
}
...
static void CB2_NewGameBirchSpeech_ReturnFromTxRandomizerChallengesOptions(void)
{
    ...
    u8 spriteId;
    u16 savedIme;
    //DebugPrintf("Returning form TX Randomizer Challenge Menu");
    //PrintTXSaveData();
...
static void Task_NewGameBirchSpeech_WaitToShowBirch(u8 taskId)
{
    ...
    {
        //DebugPrintf("Testing DebugPrintF");
...
static void MainMenu_FormatSavegamePlayer(void)
{
    ...
    if (gSaveBlock2Ptr->optionsFont == 1)
    {
        ...
        AddTextPrinterParameterized3(2, FONT_SHORT, GetStringRightAlignXOffset(FONT_NORMAL, gSaveBlock2Ptr->playerName, 100), 17, sTextColor_MenuInfo, TEXT_SKIP_DRAW, gSaveBlock2Ptr->playerName);
    }
    else
    {
...
static void MainMenu_FormatSavegamePokedex(void)
{
    ...
    if (FlagGet(FLAG_SYS_POKEDEX_GET) == TRUE)
    {
        ...
        if (gSaveBlock2Ptr->optionsFont == 1)
        {
            AddTextPrinterParameterized3(2, FONT_SHORT, GetStringRightAlignXOffset(FONT_NORMAL, str, 100), 33, sTextColor_MenuInfo, TEXT_SKIP_DRAW, str);
        }
        else
        {
...
static void MainMenu_FormatSavegameBadges(void)
{
    ...
    if (gSaveBlock2Ptr->optionsFont == 1)
    {
        AddTextPrinterParameterized3(2, FONT_SHORT, GetStringRightAlignXOffset(FONT_NORMAL, str, 0xD0), 33, sTextColor_MenuInfo, TEXT_SKIP_DRAW, str);
    }
    else
    {
