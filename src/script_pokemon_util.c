...
void CreateShinyScriptedMon(u16 species, u8 level, u16 item)
{
    ...
    CreateMonLegacy(gParties[B_TRAINER_OPPONENT_B], species, level, USE_RANDOM_IVS, TRUE, shinyPersonality, OT_ID_PLAYER_ID, 0);
    FlagClear(FLAG_IS_SHINY);

    if (item) {
        heldItem[0] = item;
        heldItem[1] = item >> 8;
        SetMonData(gParties[B_TRAINER_OPPONENT_B], MON_DATA_HELD_ITEM, heldItem);
    }
