...
//{COLOR 3}{SHADOW 3} is used for completely greyed out features.
...
static const u8 *const OptionTextRight(u8 menuItem)
{
    ...
    default:
        return sOptionMenuItemsNamesMode[menuItem];
    }
}
...
static const u8 sText_Description_Mode_Modern_Types_On[]         = _("Pokémon have the types from\nHEART AND SOUL.");
static const u8 sText_Description_Mode_Modern_Types_Off[]          = _("Pokémon have their original typing.");
...
static const u8 sText_Description_Mode_New_Stats_Off[]            = _("Pokémon maintain their original\nstats from GEN IX.");
static const u8 sText_Description_Mode_New_Stats_On[]             = _("Some Pokémon in HEART AND SOUL\nhave received stat buffs.");
...
//{COLOR 7}{COLOR 8} is used for the red text.
...
static const u8 sText_Description_Features_EasyFeebas_On[]            = _("Easier chance to find Feebas.");
static const u8 sText_Description_Features_EasyFeebas_Off[]           = _("Default chance to find Feebas.\nIt's very low.");
...
//dummy text = _("This feature is not currently\nsupported for this game.");
static const u8 sText_Description_Randomizer_Off[]                  = _("Pokémon are not randomized.");
static const u8 sText_Description_Randomizer_On[]                   = _("Pokémon are randomized.");
static const u8 sText_Description_Random_Starter_Off[]              = _("Does not randomize starter Pokémon.");
static const u8 sText_Description_Random_Starter_On[]               = _("Randomize starter Pokémon.");
static const u8 sText_Description_Random_WildPokemon_Off[]          = _("Does not randomize\nwild Pokémon.");
static const u8 sText_Description_Random_WildPokemon_On[]           = _("Randomize wild Pokémon.");
static const u8 sText_Description_Random_Trainer_Off[]              = _("Does not randomize enemy\ntrainer parties.");
static const u8 sText_Description_Random_Trainer_On[]               = _("Randomize enemy trainer parties.");
static const u8 sText_Description_Random_Static_Off[]               = _("This feature is not currently\nsupported for this game.");
static const u8 sText_Description_Random_Static_On[]                = _("Randomize static encounter Pokémon.");
static const u8 sText_Description_Random_BalanceTiers_Off[]         = _("Pokémon evolution tiers\nare not balanced.");
...
static const u8 sText_Description_Nuzlocke_Base[]               = _("Nuzlocke rules will not apply.");
...
static const u8 sText_Description_Nuzlocke_SpeciesClause_Off[]  = _("The player can catch more than\none of the same species of Pokémon..");
...
static const u8 sText_Description_Nuzlocke_ShinyClause_Off[]    = _("Shiny Pokémon are not exempt from\nthe core Nuzlocke rules.");
...
static const u8 sText_Description_Nuzlocke_Nicknaming_Off[]    = _("The player does not need to nickname\nevery Pokémon.");
static const u8 sText_Description_Nuzlocke_Nicknaming_On[]      = _("Forces the player to nickname every\nPokémon. {COLOR 7}{COLOR 8}RECOMMENDED!");
static const u8 sText_Description_Nuzlocke_Deletion_Cemetery[]  = _("Pokémon that have fainted will\nnot get deleted.");
static const u8 sText_Description_Nuzlocke_Deletion_Deletion[]  = _("Pokémon that have fainted\nwill get permanently erased!");
...
void SaveData_TxRandomizerAndChallenges(void)
{
    ...
    //MENU MODE
    gSaveBlock1Ptr->tx_Mode_AlternateSpawns             = sOptions->sel_mode[MENUITEM_MODE_ALTERNATE_SPAWNS]; //not done
    ...
    gSaveBlock1Ptr->tx_Mode_Synchronize                 = sOptions->sel_mode[MENUITEM_MODE_SYNCHRONIZE];  //not done
    ...
    gSaveBlock1Ptr->tx_Mode_Sturdy                      = sOptions->sel_mode[MENUITEM_MODE_STURDY]; //idk if this works...
    gSaveBlock1Ptr->tx_Mode_Modern_Moves                = sOptions->sel_mode[MENUITEM_MODE_MODERN_MOVES]; //not done
    gSaveBlock1Ptr->tx_Mode_Legendary_Abilities         = sOptions->sel_mode[MENUITEM_MODE_LEGENDARY_ABILITIES];
    gSaveBlock1Ptr->tx_Mode_New_Legendaries             = sOptions->sel_mode[MENUITEM_MODE_NEW_LEGENDARIES]; //not done
    //MENU FEAUTRES
    ...
    gSaveBlock1Ptr->tx_Features_PkmnDeath                   = sOptions->sel_features[MENUITEM_FEATURES_PKMN_DEATH]; //not done
    gSaveBlock1Ptr->tx_Features_Unlimited_WT                = sOptions->sel_features[MENUITEM_FEATURES_UNLIMITED_WT]; //not done
    // MENU_RANDOMIZER
    if (sOptions->sel_randomizer[MENUITEM_RANDOM_OFF_ON] == TRUE)
    {
        ...
        gSaveBlock1Ptr->tx_Random_Similar            = !sOptions->sel_randomizer[MENUITEM_RANDOM_SIMILAR_EVOLUTION_LEVEL];//not done
        gSaveBlock1Ptr->tx_Random_MapBased           = TX_RANDOM_MAP_BASED;
        gSaveBlock1Ptr->tx_Random_IncludeLegendaries = sOptions->sel_randomizer[MENUITEM_RANDOM_INCLUDE_LEGENDARIES]; 
        //the code is there for this and below, but I haven't tested it yet.
    ...
    // MENU_DIFFICULTY
    gSaveBlock1Ptr->tx_Challenges_PartyLimit    = sOptions->sel_difficulty[MENUITEM_DIFFICULTY_PARTY_LIMIT]; //not done
    ...
    gSaveBlock1Ptr->tx_Features_LimitDifficulty              = sOptions->sel_difficulty[MENUITEM_DIFFICULTY_LIMIT_DIFFICULTY]; //not done
    gSaveBlock1Ptr->tx_Challenges_MaxPartyIVs                         = sOptions->sel_difficulty[MENUITEM_DIFFICULTY_MAX_PARTY_IVS]; //not done
    // MENU_CHALLENGES
    gSaveBlock1Ptr->tx_Challenges_EvoLimit             = sOptions->sel_challenges[MENUITEM_CHALLENGES_EVO_LIMIT]; //not done
...
static void DrawChoices_Random_EvoStages(int selection, int y)
{
    ...
    {
        gSaveBlock1Ptr->tx_Random_Chaos = 1;
    }
}
static void DrawChoices_Random_Legendaries(int selection, int y)
{
...
static void DrawChoices_Challenges_Nuzlocke(int selection, int y)
{
    ...
    {
        if (selection == 2)
            gSaveBlock1Ptr->tx_Challenges_NuzlockeHardcore = 1;
        gSaveBlock1Ptr->tx_Challenges_Nuzlocke = 1;
        FlagSet(FLAG_NUZLOCKE);
    }
...
static void DrawChoices_Nuzlocke_SpeciesClause(int selection, int y)
{
    ...
    if (selection == 0)
    {
        gSaveBlock1Ptr->tx_Nuzlocke_SpeciesClause = 0;
    }
    else
    {
        gSaveBlock1Ptr->tx_Nuzlocke_SpeciesClause = 1;
    }
}
static void DrawChoices_Nuzlocke_ShinyClause(int selection, int y)
{
    ...
    if (selection == 0)
    {
        gSaveBlock1Ptr->tx_Nuzlocke_ShinyClause = 0;
    }
    else
    {
        gSaveBlock1Ptr->tx_Nuzlocke_ShinyClause = 1;
    }
}
...
static void DrawChoices_Nuzlocke_Nicknaming(int selection, int y)
{
    ...
    if (selection == 0)
    {
        gSaveBlock1Ptr->tx_Nuzlocke_Nicknaming = 0;
    }
    else
    {
        gSaveBlock1Ptr->tx_Nuzlocke_Nicknaming = 1;
    }
}
...
static void DrawChoices_Nuzlocke_Deletion(int selection, int y)
{
    ...
    if (selection == 0)
    {
        gSaveBlock1Ptr->tx_Nuzlocke_Deletion = 0;
    }
    else
    {
        gSaveBlock1Ptr->tx_Nuzlocke_Deletion = 1;
    }
}
...
static void DrawChoices_Challenges_ScalingIVs(int selection, int y)
{
    ...
    {
        gSaveBlock1Ptr->tx_Challenges_TrainerScalingIVs = 0;
    }
...
static void DrawChoices_Challenges_ScalingEVs(int selection, int y)
{
    ...
    {
        gSaveBlock1Ptr->tx_Challenges_TrainerScalingEVs = 0;
    }
}
...
static const u8 gTypeNames[NUMBER_OF_MON_TYPES][TYPE_NAME_LENGTH + 1] =
{
