...
void CB2_WhiteOut(void)
{
    ...
    if (++gMain.state >= 120)
    {
        ...
        ScriptContext_Init();
        UnlockPlayerFieldControls();
        if (gSaveBlock1Ptr->tx_Nuzlocke_Deletion == 1) //you can only white out if you have 0 alive mons
        {
            gFieldCallback = FieldCB_RushInjuredPokemonToCenter;
        }      
        if (IsWhiteoutCutscene())
            gFieldCallback = FieldCB_RushInjuredPokemonToCenter;
        if (gSaveBlock1Ptr->tx_Nuzlocke_Deletion == 0) //for some reason setting this to "else" made two scripts trigger at the same time.
        //Hence, the checks for "gSaveBlock1Ptr->tx_Nuzlocke_Deletion" if it's set to 0 or 1.
        {
            gFieldCallback = FieldCB_WarpExitFadeFromBlack;
        }
        state = 0;
...
static const u8 sMapsecToRegion[MAPSEC_COUNT] = {
    ...
    //[MAPSEC_HOENN_ROUTE]                    = REGION_HOENN,
