...
bool32 CanEndureHit(enum BattlerId battler, enum BattlerId battlerTarget, enum Move move)
{
    ...
    {
        if ((gSaveBlock1Ptr->tx_Mode_Sturdy == 1) && gAiLogicData->abilities[battlerTarget] == ABILITY_STURDY)
            return TRUE;
...
enum AIConsiderGimmick ShouldTeraFromCalcs(enum BattlerId battler, enum BattlerId opposingBattler, struct AltTeraCalcs *altCalcs)
{
    ...
    if (TRAINER_BATTLE_PARAM.opponentA == TRAINER_BRIE_1)  //we want Brie to terastallize
        {
            return USE_GIMMICK;
        } 
    if (TRAINER_BATTLE_PARAM.opponentA == TRAINER_SHREK)  //we want Shrek to terastallize
        {
            return USE_GIMMICK;
        }
