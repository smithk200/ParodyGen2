...
static const u8 sText_WildPkmnAppeared_LA[] = _("You encountered a wild {B_OPPONENT_MON1_NAME}!\nRun? {L_BUTTON}+{R_BUTTON}+{A_BUTTON}.\p");
...
const u8 *const gBattleStringsTable[STRINGID_COUNT] =
{
    ...
    [STRINGID_MON_DIED]                             = COMPOUND_STRING("{B_SCR_NAME_WITH_PREFIX} died!\p"),
    ...
    [STRINGID_HITXTIMES]                            = COMPOUND_STRING("The Pokémon was hit {B_BUFF1} times!"), //SV has dynamic plural here
    [STRINGID_HITXTIME]                            = COMPOUND_STRING("The Pokémon was hit {B_BUFF1} time!"),
    ...
    [STRINGID_PKMNAVOIDEDATTACK]                    = COMPOUND_STRING("{B_SCR_NAME_WITH_PREFIX}'s attack missed!"),
    [STRINGID_BATTLERAVOIDEDATTACK]                 = COMPOUND_STRING("{B_DEF_NAME_WITH_PREFIX}'s attack missed!"),
    ...
    [STRINGID_PKMNIDENTIFIED]                       = COMPOUND_STRING("{B_DEF_NAME_WITH_PREFIX}'s evasiveness went back to normal!\nGhosts can be hit now!"),
    ...
    [STRINGID_PKMNTERAS]                           = COMPOUND_STRING("Time to Terastallize!"),
    [STRINGID_MEGAEVOEVOLVED_CORRUPT]               = COMPOUND_STRING("{B_ATK_NAME_WITH_PREFIX} has Mega Evolved into Corrupt {B_BUFF1}!"),
};
...
static const u8 sText_TwoTrainersWantToBattleNuzlocke[] = _("{B_TRAINER1_NAME_WITH_CLASS} and {B_TRAINER2_NAME_WITH_CLASS}\nare trying to end your run!\p");
    ...
    case STRINGID_INTROMSG: // first battle msg
        if (gBattleTypeFlags & BATTLE_TYPE_TRAINER)
        {
            ...
            else
            {
                if (BATTLE_TWO_VS_ONE_OPPONENT)
                    ...
                    stringPtr = sText_TwoTrainersWantToBattle;
                else if (IsNuzlockeActive() && (gBattleTypeFlags & BATTLE_TYPE_TWO_OPPONENTS))
                    stringPtr = sText_TwoTrainersWantToBattleNuzlocke;
        ...
        else
        {
            if (IsGhostBattleWithoutScope())
                ...
                stringPtr = sText_WildPkmnAppeared_LA;
            else if (gSaveBlock2Ptr->optionsRunType == 2) //unused
...
static const u8 *BattleStringGetOpponentNameByTrainerId(u16 trainerId, u8 *text, u8 multiplayerId, enum BattlerId battler)
{
    ...
    else
    {
        ...
        if (trainerClass == TRAINER_CLASS_RIVAL_EARLY_FRLG || trainerClass == TRAINER_CLASS_RIVAL_LATE_FRLG || trainerClass == TRAINER_CLASS_CHAMPION_FRLG || trainerClass == TRAINER_CLASS_RIVAL)
