...
static void LoadBattleEnvironmentGfx(u16 environment)
{
    ...
    {
        if (environment >= NELEMS(gBattleEnvironmentInfo2))
            environment = BATTLE_ENVIRONMENT_PLAIN;  // If higher than the number of entries in gBattleEnvironmentInfo, use the default.
        // Copy to bg3
        DecompressDataWithHeaderVram(gBattleEnvironmentInfo2[environment].background.tileset, (void *)(BG_CHAR_ADDR(2)));
        DecompressDataWithHeaderVram(gBattleEnvironmentInfo2[environment].background.tilemap, (void *)(BG_SCREEN_ADDR(26)));
        LoadPalette(gBattleEnvironmentInfo2[environment].palette, BG_PLTT_ID(2), 3 * PLTT_SIZE_4BPP);
    }
    else
    {
